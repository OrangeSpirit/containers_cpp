...
public:
    map() : Tree<K, V>() {};
    map(std::initializer_list<typename Tree<K, V>::value_type> const& items) : Tree<K, V>(items) {};
    ...
    V& operator[](const K& key) {
        if (!this->contains(key)) {
            ...
            return const_cast<V&>(res_it.first->second);
        } else {
            auto it = this->begin();
            for(; it != this->end(); ++it) {
                if (it->first == key) return const_cast<V&>(it->second);
            }
    ...
    std::pair<typename Tree<K, V>::Iterator, bool> insert(const typename Tree<K, V>::value_type& value) noexcept override {
        return Tree<K, V>::insert(value);
    }
    
    std::pair<typename Tree<K, V>::Iterator, bool> insert(const K& key, const V& obj) {
    ...
    std::pair<typename Tree<K, V>::Iterator, bool> insert_or_assign(const K& key, const V& obj) {
...
private:
    ...
    void At_(const K& key, V& res, map<K, V>* tr) const {
        if (!tr->root_ || (!tr->left_ && !tr->right_ && tr->root_->element_.first != key)) {
...
//   s21_map.insert(pair4);
//   it = s21_map.end();
//   EXPECT_EQ(s21_map.end()->first, "hola-hola");
//   EXPECT_EQ(s21_map.size(), 5U);
// }
...
TEST(map_insert_many, case2) {
  ...
  std::vector<std::pair<s21::map<double, std::string>::iterator, bool>>
      emplace1 = s21_map.insert_many(pair1, pair2, pair3);
...
TEST(map_insert_many, case5) {
  ...
  std::vector<std::pair<s21::map<double, std::string>::iterator, bool>>
      emplace1 = s21_map.insert_many(pair1);
